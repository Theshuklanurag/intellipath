...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: 'syllabus',    icon: BookOpen,     label: 'Syllabus Manager' },
...
function AnalyticsPage() {
  ...
  const [syllabus, setSyllabus] = useState(ls('ip_teacher_syllabus', []))

  useEffect(() => {
    getTeacherData().then(r => setData(r.data || { students: [] })).catch(() => {})
  }, [])
  ...
  const subjectAvgData = subjects.map(sub => {
    ...
    return {
      subject: sub.slice(0, 10),
      avg: subMarks.length > 0 ? Math.round(subMarks.reduce((a, b) => a + Number(b.grade), 0) / subMarks.length) : 0,
  ...
  const syllabusData = syllabus.map(s => ({
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        ...
        <div className="chart-card">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={subjectAvgData.length > 0 ? subjectAvgData : [{ subject: 'No data', avg: 0 }]}>
        ...
        <div className="chart-card">
          ...
          <div className="chart-subtitle">Present vs absent records</div>
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={attendancePie.some(d => d.value > 0) ? attendancePie : [{ name: 'No records', value: 1 }]}
                cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value" paddingAngle={4}>
                <Cell fill="#00FF88" />
                <Cell fill="#FF4D6A" />
              </Pie>
              <Tooltip formatter={(val) => [val, 'Records']} contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, fontSize: 12 }} />
        ...
        <div className="chart-card">
          <div className="chart-title">Grade Distribution Histogram</div>
          <div className="chart-subtitle">Number of students per grade range</div>
          <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="chart-card">
          ...
          <div className="chart-subtitle">Topic completion percentage</div>
...
function StudentsPage() {
  ...
  const addStudent = async () => {
    if (!form.name || !form.course) return toast.error('Name and course are required')
    try {
  ...
  const removeStudent = async (id) => {
    try {
      ...
      setStudents(Array.isArray(res.data) ? res.data : students.filter(s => s.id !== id))
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        <input className="inp flex-1 min-w-48" placeholder="Search by name, course, roll no..."
      ...
      {filtered.length > 0 ? (
        <div className="glass rounded-2xl overflow-hidden">
          <table className="erp-table">
            <thead>
              <tr><th>#</th><th>Student</th><th>Course</th><th>Roll No</th><th>Year</th><th>Contact</th><th>Action</th></tr>
            </thead>
            <tbody>
              {filtered.map((s, i) => (
                <tr key={s.id || i}>
...
const save = async () => {
  ...
  // Stats
  const subjects = [...new Set(attendance.map(a => a.subject))].filter(Boolean)
  ...
  const trendData = dates.map(d => {
    ...
    return { date: d.slice(5), rate: recs.length > 0 ? Math.round((presentCount / recs.length) * 100) : 0 }
  })
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto">
      ...
      <div className="tab-group mb-5">
        {[{ key: 'mark', label: '✅ Mark Attendance' }, { key: 'records', label: '📋 Records' }, { key: 'chart', label: '📊 Trend Chart' }].map(t => (
      ...
      {tab === 'mark' && (
        <div className="space-y-4">
          ...
          <div className="glass rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs mb-1.5" style={{ color: 'var(--text-muted)' }}>Subject / Period</label>
              <input className="inp" placeholder="e.g. Data Structures - Lecture 1"
                value={subject} onChange={e => setSubject(e.target.value)} />
            </div>
            <div className="flex items-end gap-2">
              <button onClick={() => markAll('present')} className="btn-secondary text-xs flex-1" style={{ color: 'var(--green)', borderColor: 'rgba(0,255,136,0.2)' }}>
                All Present
              </button>
              <button onClick={() => markAll('absent')} className="btn-secondary text-xs flex-1" style={{ color: 'var(--red)', borderColor: 'rgba(255,77,106,0.2)' }}>
                All Absent
              </button>
            </div>
          </div>
          ...
          {students.length > 0 ? (
            <div className="glass rounded-2xl overflow-hidden">
              <table className="erp-table">
                <thead><tr><th>Student</th><th>Roll No</th><th>Course</th><th>Status</th></tr></thead>
                <tbody>
                  {students.map((s, i) => {
                    ...
                    return (
                      <tr key={s.id || i}>
                        ...
                        <td><span className="badge badge-cyan" style={{ fontSize: 10 }}>{s.course}</span></td>
                        <td>
                          <div className="flex gap-2">
                            <button onClick={() => setSession(sess => ({ ...sess, [s.id]: 'present' }))}
                              className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all"
                              style={{
                                background: status === 'present' ? 'var(--green-dim)' : 'var(--bg-card)',
                                color: status === 'present' ? 'var(--green)' : 'var(--text-muted)',
                                border: `1px solid ${status === 'present' ? 'rgba(0,255,136,0.3)' : 'var(--border)'}`
                              }}>
                              <Check className="w-3 h-3" /> P
                            </button>
                            <button onClick={() => setSession(sess => ({ ...sess, [s.id]: 'absent' }))}
                              className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all"
                              style={{
                                background: status === 'absent' ? 'var(--red-dim)' : 'var(--bg-card)',
                                color: status === 'absent' ? 'var(--red)' : 'var(--text-muted)',
                                border: `1px solid ${status === 'absent' ? 'rgba(255,77,106,0.3)' : 'var(--border)'}`
                              }}>
            ...
            <div className="flex flex-col items-center justify-center py-12" style={{ color: 'var(--text-muted)' }}>
              ...
              <p className="text-sm">Add students first to mark attendance</p>
              <Link to="/teacher/students" className="mt-2 text-sm" style={{ color: 'var(--cyan)' }}>Go to Students →</Link>
            </div>
          )}

          {students.length > 0 && (
            <button onClick={save} className="btn-primary w-full" style={{ fontSize: 14 }}>
      ...
      {tab === 'records' && (
        <div className="space-y-4">
          <div className="flex gap-3 mb-4 flex-wrap">
            {subjects.map(sub => (
              <span key={sub} className="badge badge-cyan cursor-pointer">{sub}</span>
            ))}
          </div>
          {attendance.length > 0 ? (
            <div className="glass rounded-2xl overflow-hidden">
                ...
                <tbody>
                  {[...attendance].reverse().slice(0, 50).map((rec, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ color: 'var(--text-primary)', fontFamily: 'Outfit', fontWeight: 500, fontSize: 13 }}>{rec.studentName}</td>
                      <td>
                        <span className={`badge ${rec.status === 'present' ? 'badge-green' : 'badge-red'}`} style={{ fontSize: 10 }}>
...
function MarksPage() {
 ...
 const saveMark = async () => {
  ...
  const subjects = [...new Set(marks.map(m => m.subject))].filter(Boolean)
  const filtered = filterSubject ? marks.filter(m => m.subject === filterSubject) : marks

  const getGradeStyle = (g, max = 100) => {
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto">
      <div className="page-header">
        ...
        <p className="page-subtitle">{marks.length} grade records across all subjects</p>
      </div>
      ...
      {tab === 'enter' && (
        <div className="space-y-4">
          <div className="glass rounded-2xl p-5">
            <h3 className="font-syne font-semibold text-sm mb-4" style={{ color: 'var(--text-primary)' }}>Enter Marks</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              ...
              {[
                ...
                { key: 'examName', label: 'Exam/Test',   placeholder: 'e.g. Mid Term, Unit Test 1' },
                ...
                { key: 'remarks',  label: 'Remarks',        placeholder: 'Optional teacher remarks' },
              ].map(f => (
                <div key={f.key}>
                  <label className="block text-xs mb-1.5" style={{ color: 'var(--text-muted)' }}>{f.label}</label>
            ...
            {form.grade && form.maxGrade && (
              <div className="flex items-center gap-3 mb-4 p-3 rounded-xl" style={{ background: 'var(--bg-secondary)' }}>
                ...
                <div className="grade-pill" style={{
                  background: getGradeStyle(form.grade, form.maxGrade).bg,
                  color: getGradeStyle(form.grade, form.maxGrade).color
                }}>
                  {getGradeStyle(form.grade, form.maxGrade).grade} — {form.grade}/{form.maxGrade} ({Math.round((Number(form.grade)/Number(form.maxGrade))*100)}%)
                </div>
              </div>
            )}

            <button onClick={saveMark} className="btn-primary text-sm">
              <Save className="w-4 h-4" /> Record Mark
            </button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="flex gap-2 flex-wrap mb-2">
            <button onClick={() => setFilterSubject('')}
              className={`tab ${filterSubject === '' ? 'active' : ''}`} style={{ fontSize: 11 }}>All</button>
            {subjects.map(sub => (
              <button key={sub} onClick={() => setFilterSubject(sub)}
                className={`tab ${filterSubject === sub ? 'active' : ''}`} style={{ fontSize: 11 }}>{sub}</button>
            ))}
          </div>
          {filtered.length > 0 ? (
            <div className="glass rounded-2xl overflow-hidden">
              <table className="erp-table">
                <thead><tr><th>Date</th><th>Student</th><th>Subject</th><th>Exam</th><th>Score</th><th>Grade</th><th>Remarks</th><th></th></tr></thead>
                <tbody>
                  {[...filtered].reverse().map(m => {
                    const gs = getGradeStyle(m.grade, m.maxGrade)
                    return (
                      <tr key={m.id}>
                        <td style={{ fontFamily: 'JetBrains Mono', fontSize: 11, color: 'var(--text-muted)' }}>{m.date}</td>
                        <td style={{ color: 'var(--text-primary)', fontFamily: 'Outfit', fontWeight: 500, fontSize: 13 }}>{m.studentName}</td>
                        <td><span className="badge badge-cyan" style={{ fontSize: 10 }}>{m.subject}</span></td>
                        <td style={{ fontSize: 12 }}>{m.examName || '—'}</td>
                        <td>
                          <span style={{ fontFamily: 'JetBrains Mono', fontSize: 13, color: gs.color, fontWeight: 700 }}>
                            {m.grade}/{m.maxGrade}
                          </span>
                        </td>
                        <td>
                          <div className="grade-pill" style={{ background: gs.bg, color: gs.color, fontSize: 12, minWidth: 'auto', padding: '2px 10px' }}>
                            {gs.grade} ({Math.round((Number(m.grade)/Number(m.maxGrade))*100)}%)
                          </div>
                        </td>
                        <td style={{ fontSize: 12, color: 'var(--text-muted)' }}>{m.remarks || '—'}</td>
                        <td>
                          <button onClick={() => deleteMark(m.id)} className="btn-ghost" style={{ color: 'var(--red)' }}>
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </td>
                      </tr>
      ...
      {tab === 'report' && (
        <div className="space-y-4">
          {students.length > 0 ? students.map(s => {
            ...
            const avg = sMarks.length > 0 ? Math.round(sMarks.reduce((a, b) => a + (Number(b.grade)/Number(b.maxGrade))*100, 0) / sMarks.length) : null
            const gs = avg !== null ? getGradeStyle(avg) : null
            return (
              <div key={s.id} className="glass rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{s.course} · {s.rollNo || 'No roll no'}</p>
                    </div>
                  </div>
                  {gs && (
                    <div className="grade-pill text-lg" style={{ background: gs.bg, color: gs.color }}>
                      {gs.grade} — {avg}%
                    </div>
                  )}
                </div>
                {sMarks.length > 0 ? (
                  <div className="space-y-2">
                    {sMarks.map(m => {
                      const pct = Math.round((Number(m.grade)/Number(m.maxGrade))*100)
                      const mgs = getGradeStyle(m.grade, m.maxGrade)
                      return (
                        <div key={m.id} className="flex items-center gap-3">
                          <span className="text-xs w-32 truncate" style={{ color: 'var(--text-muted)' }}>{m.subject}</span>
                          <span className="text-xs w-24" style={{ color: 'var(--text-muted)' }}>{m.examName || '—'}</span>
            ...
            <div className="flex flex-col items-center justify-center py-12" style={{ color: 'var(--text-muted)' }}>
              <Award className="w-10 h-10 mb-3 opacity-20" />
              <p className="text-sm">Add students first to see report cards</p>
            </div>
          )}
        </div>
      )}
...
function StudentProgressPage() {
  ...
  const sMarks = marks.filter(m => m.studentId === selected)
  const sAttendance = attendance.filter(a => a.studentId === selected)
  const attendRate = sAttendance.length > 0
    ? Math.round((sAttendance.filter(a => a.status === 'present').length / sAttendance.length) * 100) : 0

  const gradeHistory = sMarks.map(m => ({
    exam: m.examName || m.subject,
    grade: Math.round((Number(m.grade)/Number(m.maxGrade))*100),
    subject: m.subject
  }))
...
Student: ${student.name}, Course: ${student.course}, Year: ${student.year || 'N/A'}

Grade Records: ${JSON.stringify(sMarks)}
Attendance: ${attendRate}% (${sAttendance.filter(a => a.status === 'present').length} present out of ${sAttendance.length})
...
5. Specific recommendations for the student
6. Predicted final grade if trend continues
7. Suggested study plan adjustments`
      })
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto">
      <div className="page-header">
        ...
        <p className="page-subtitle">Individual student performance tracking and AI reports</p>
      </div>

      {students.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16" style={{ color: 'var(--text-muted)' }}>
          <Users className="w-12 h-12 mb-4 opacity-20" />
          <p className="text-sm">Add students first to track their progress</p>
          <Link to="/teacher/students" className="mt-2 text-sm" style={{ color: 'var(--cyan)' }}>Go to Students →</Link>
        </div>
      ) : (
          ...
          <div className="glass rounded-2xl overflow-hidden">
            ...
            <div className="p-2">
              {students.map(s => {
                ...
                const avg = sm.length > 0 ? Math.round(sm.reduce((a, b) => a + (Number(b.grade)/Number(b.maxGrade))*100, 0) / sm.length) : 0
                return (
                  <button key={s.id} onClick={() => { setSelected(s.id); setAiReport('') }}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {avg > 0 && <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{avg}% avg</p>}
                    </div>
                  </button>
                )
              })}
              ...
              <>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{student.course} · {student.year || ''} · {student.rollNo || ''}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        ...
                        { label: 'Attendance', value: sAttendance.length > 0 ? `${attendRate}%` : '—', color: attendRate >= 75 ? 'var(--green)' : 'var(--red)' },
                ...
                {gradeHistory.length > 0 && (
                  <div className="chart-card">
                    ...
                    <div className="chart-subtitle">Performance across all tests</div>
                    <ResponsiveContainer width="100%" height={180}>
                      <LineChart data={gradeHistory}>
                        ...
                        <YAxis tick={{ fill: 'var(--text-muted)', fontSize: 10 }} axisLine={false} tickLine={false} domain={[0, 100]} />
                        <Tooltip content={<CT />} />
                        <Line type="monotone" dataKey="grade" name="Grade %" stroke="#9B59FF" strokeWidth={2.5} dot={{ fill: '#9B59FF', r: 4 }} activeDot={{ r: 6 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                )}
                ...
                <div className="glass rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Detailed AI analysis of {student.name}'s performance</p>
                    </div>
                  ...
                  {aiReport ? (
                    ...
                    <div className="flex items-center justify-center py-8" style={{ color: 'var(--text-muted)', fontSize: 13 }}>
                      Click "Generate Report" to get AI analysis
                    </div>
                  )}
                </div>
              </>
...
function SyllabusPage() {
  ...
  const updateCoverage = (id, val) => {
    const u = syllabus.map(s => s.id === id ? { ...s, coverage: Number(val) } : s)
    setSyllabus(u); lsSet('ip_teacher_syllabus', u)
  }

  const subjects = [...new Set(syllabus.map(s => s.subject))].filter(Boolean)
  const filtered = filterSubject ? syllabus.filter(s => s.subject === filterSubject) : syllabus
  const totalCoverage = filtered.length > 0 ? Math.round(filtered.reduce((a, b) => a + Number(b.coverage), 0) / filtered.length) : 0
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto">
      <div className="page-header">
        ...
        <p className="page-subtitle">Track and manage topic-wise syllabus coverage</p>
      </div>

      {/* Add form */}
      <div className="glass rounded-2xl p-5 mb-5">
        <h3 className="font-syne font-semibold text-sm mb-4" style={{ color: 'var(--text-primary)' }}>
          {editId ? '✏️ Edit Topic' : '➕ Add Syllabus Topic'}
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
          {[
            { key: 'subject', label: 'Subject*', placeholder: 'e.g. Data Structures' },
            { key: 'unit',    label: 'Unit/Module', placeholder: 'e.g. Unit 3' },
          ...
          <div>
            ...
            <input type="range" min="0" max="100" value={form.coverage}
              onChange={e => setForm(p => ({ ...p, coverage: Number(e.target.value) }))} className="w-full" />
          </div>
          <div>
        ...
        <div className="flex gap-3">
          ...
          {editId && <button onClick={() => { setEditId(null); setForm({ subject: '', unit: '', topic: '', coverage: 0, notes: '', status: 'pending' }) }} className="btn-secondary text-sm">Cancel</button>}
        </div>
      </div>
      ...
      {syllabus.length > 0 && (
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <button onClick={() => setFilterSubject('')} className={`tab ${filterSubject === '' ? 'active' : ''}`} style={{ fontSize: 11 }}>All ({syllabus.length})</button>
          {subjects.map(sub => (
            <button key={sub} onClick={() => setFilterSubject(sub)} className={`tab ${filterSubject === sub ? 'active' : ''}`} style={{ fontSize: 11 }}>
              {sub}
            </button>
          ))}
          <div className="ml-auto flex items-center gap-2 text-sm font-syne font-bold" style={{ color: 'var(--cyan)' }}>
            Avg Coverage: {totalCoverage}%
          </div>
        </div>
      )}
      ...
      {filtered.length > 0 ? (
            ...
            <tbody>
              {filtered.map(s => (
                <tr key={s.id}>
                  ...
                  <td>
                    <span style={{ fontSize: 11, fontWeight: 600, color: statusColors[s.status] }}>{s.status}</span>
                  </td>
                  <td>
                    <div className="flex items-center gap-2">
                      <input type="range" min="0" max="100" value={s.coverage}
                        onChange={e => updateCoverage(s.id, e.target.value)}
                        className="w-16" style={{ height: 4 }} />
                      <span className="font-syne font-bold text-xs" style={{ color: s.coverage >= 80 ? 'var(--green)' : s.coverage >= 40 ? 'var(--amber)' : 'var(--red)' }}>
                        {s.coverage}%
                      </span>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className="flex gap-1">
                      <button onClick={() => startEdit(s)} className="btn-ghost" style={{ color: 'var(--cyan)' }}>
                        <Edit3 className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => del(s.id)} className="btn-ghost" style={{ color: 'var(--red)' }}>
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  </td>
        ...
        <div className="flex flex-col items-center justify-center py-12" style={{ color: 'var(--text-muted)' }}>
          ...
          <p className="text-sm">No syllabus topics yet. Add your first one above!</p>
        </div>
      )}
    </div>
  )
...
function AssignmentsPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setAssignments(prev => [...prev, ...(Array.isArray(res.data) ? [] : [])])
  ...
  return (
    <div className="p-5 max-w-4xl mx-auto">
      <div className="page-header">
        ...
        <p className="page-subtitle">Create, manage and distribute assignments</p>
      </div>
      ...
      {tab === 'create' && (
        <div className="glass rounded-2xl p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { key: 'title',    label: 'Assignment Title*', placeholder: 'e.g. Implement AVL Tree' },
              { key: 'subject',  label: 'Subject*',          placeholder: 'e.g. Data Structures' },
          ...
          <div>
            ...
            <textarea className="inp resize-none w-full" rows={4} placeholder="Step-by-step instructions, evaluation criteria, submission format..."
              ...
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {a.due_date && <span className="badge badge-amber" style={{ fontSize: 10 }}>Due: {a.due_date}</span>}
...
// ─── AI PAGES (Question Paper, Lesson Plan, AI Grader) ────
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="space-y-4">
          <textarea className="inp resize-none w-full" rows={12}
            placeholder="Describe your paper requirements:&#10;- Subject, year, exam type&#10;- Number of questions per type&#10;- Topics to cover&#10;- Total marks&#10;&#10;Example: Create a 100-mark B.Tech 3rd year Data Structures paper with 10 MCQs (20 marks), 5 short answers (30 marks), 3 long questions (50 marks). Cover Arrays, Trees, Graphs, Sorting."
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto">
      <div className="page-header">
        ...
        <p className="page-subtitle">Generate detailed structured lesson plans with AI</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="space-y-4">
          {[
...
function AIGraderPage() {
  ...
  const grade = async () => {
    ...
    try {
      const res = await API.post('/ai/chatbot', {
        prompt: `You are an expert AI assignment grader for college-level courses.
...
Grade this submission comprehensively. Provide in Markdown:
## Score: [X]/100
...
[Detailed bullet points with what was expected]

## 📊 Concept-wise breakdown
[Table with concept, marks awarded, max marks, feedback]

## 💡 Personalized feedback
[3-4 specific, actionable improvement suggestions]

## 📈 Predicted grade if this trend continues
[Honest assessment]`
      })
      setOutput(res.data.output)
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto">
      <div className="page-header">
        ...
        <p className="page-subtitle">Auto-grade with detailed feedback — save hours of manual checking</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--green)' }}>✅ Answer Key / Model Answer</label>
            <textarea className="inp resize-none w-full" rows={10} placeholder="Paste the correct answer or marking scheme..."
  ...
  return (
    <div className="p-5 max-w-3xl mx-auto">
      <div className="page-header">
        ...
        <p className="page-subtitle">Post notices and announcements for your students</p>
      </div>
      ...
      <div className="glass rounded-2xl p-5 mb-5 space-y-4">
        ...
        <div>
          ...
          <textarea className="inp resize-none w-full" rows={4} placeholder="Write your announcement..." value={message} onChange={e => setMessage(e.target.value)} />
        </div>
            ...
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="badge" style={{ fontSize: 10, background: priorityColors[a.priority] + '15', color: priorityColors[a.priority], border: `1px solid ${priorityColors[a.priority]}30` }}>
                    {a.priority}
                  </span>
    ...
    <div className="p-5 max-w-2xl mx-auto">
      ...
      <div className="glass rounded-2xl p-5 space-y-4">
        {[
          ...
          { key: 'subjectsTaught',  label: 'Subjects Taught',      placeholder: 'e.g. Data Structures, DBMS, OS' },
        ...
        <div>
          ...
          <textarea className="inp resize-none w-full" rows={4}
      ...
      <div className="glass rounded-2xl p-4 mb-5">
        <div className="flex gap-3">
          <textarea className="inp flex-1 resize-none" rows={2}
            placeholder="e.g. Schedule Data Structures for B.Tech 3rd year MWF 10-11am, DBMS for 2nd year T/Th 2-3pm..."
            value={prompt} onChange={e => setPrompt(e.target.value)} />
          ...
          {playlists.map((p, i) => (
            <div key={p.id || i} className="glass rounded-2xl overflow-hidden transition-all duration-200"
              ...
              <div className="h-32 flex items-center justify-center relative" style={{ background: 'var(--bg-secondary)' }}>
                ...
                {p.subject && (
                  <span className="absolute bottom-2 left-3 badge badge-cyan" style={{ fontSize: 10 }}>{p.subject}</span>
                )}
              </div>
              <div className="p-4">
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg-primary)' }}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-60 flex flex-col transition-transform duration-300 lg:relative lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        ...
        <div className="flex items-center gap-2.5 h-14 px-4 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-3 space-y-4">
          {NAV_GROUPS.map(group => (
            <div key={group.label}>
              <p className="px-2 mb-1.5 text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>
                {group.label}
              </p>
        ...
        <header className="h-14 flex items-center justify-between px-5 flex-shrink-0"
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl"
              ...
              <Activity className="w-3.5 h-3.5" style={{ color: 'var(--pink)' }} />
