...
import {
  ...
  TrendingUp, Award, Zap, ArrowUpRight, Clock
} from 'lucide-react'
import {
...
import { getAcademicDetails, getStudyLogs } from '../../services/api'
...
export default function DashboardHome() {
  ...
  useEffect(() => {
    Promise.all([getAcademicDetails(), getStudyLogs()])
      .then(([ac, sl]) => {
        const academic = ac.data
        setAcademic(academic)

        // Build grade data from subjects
        if (academic?.subjects?.length > 0) {
          setGradeData(academic.subjects.map(s => ({
        ...
        logs.forEach(log => {
          if (log.date) {
            const dayName = dayMap[new Date(log.date).getDay()]
            if (dayName) hoursMap[dayName] = (hoursMap[dayName] || 0) + Number(log.hours)
          }
        })

        setStudyHours(days.map(d => ({ day: d, hours: hoursMap[d] || 0 })))
      })
      .catch(() => {})
  ...
  const avgGrade       = gradeData.length > 0
    ? Math.round(gradeData.reduce((a, b) => a + b.grade, 0) / gradeData.length) : 0
  const avgAttendance  = gradeData.length > 0
    ? Math.round(gradeData.reduce((a, b) => a + b.attendance, 0) / gradeData.length) : 0
  ...
  return (
    <div className="p-5 max-w-7xl mx-auto">
      ...
      <div className="mb-6 rounded-2xl p-6 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg,rgba(0,212,255,0.08) 0%,rgba(155,89,255,0.05) 100%)',
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
            {totalSubjects > 0
              ? `${totalSubjects} subjects tracked · ${avgGrade > 0 ? `${avgGrade}% avg grade` : 'No grades yet'} · 12 AI tools ready`
              : 'Start adding subjects in Academic Tracker · 12 AI tools ready'}
          </p>
        </div>
      </div>
        ...
        {[
          {
            ...
            color: 'var(--cyan)',
            icon: TrendingUp,
            trend: totalSubjects > 0 ? `${totalSubjects} subjects` : 'No subjects yet'
          },
          {
            label: 'Attendance',
            ...
            color: 'var(--green)',
            icon: Award,
            trend: avgAttendance >= 75 ? 'Good standing' : avgAttendance > 0 ? 'Needs attention' : 'Not tracked'
          },
          {
            label: 'Study Hours',
            value: totalStudyHours > 0 ? `${totalStudyHours}h` : '—',
            color: 'var(--amber)',
            icon: Clock,
            trend: 'This week'
          },
          ...
          return (
            <div key={i} className="stat-card">
              ...
              <div className="stat-trend mt-1" style={{ color: 'var(--text-muted)', fontSize: 11 }}>{s.trend}</div>
            </div>
          )
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="lg:col-span-2 chart-card">
          ...
          <div className="chart-subtitle">
            {totalStudyHours > 0
              ? `${totalStudyHours}h logged this week`
              : 'Log study sessions in Progress Tracker to see data here'}
          </div>
        ...
        <div className="chart-card flex flex-col items-center justify-center">
          ...
          <ResponsiveContainer width="100%" height={140}>
            <RadialBarChart
              ...
              data={[{ value: avgGrade || 0, fill: '#00D4FF' }]}
              startAngle={90} endAngle={-270}>
              <RadialBar
                dataKey="value" cornerRadius={6} fill="#00D4FF"
                background={{ fill: 'rgba(0,212,255,0.05)' }}
              />
            </RadialBarChart>
          </ResponsiveContainer>
          <div className="text-center -mt-4">
            <span className="font-syne font-bold text-3xl" style={{ color: 'var(--cyan)' }}>
              {avgGrade || 0}%
            </span>
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
              {avgGrade > 0 ? 'Performance Score' : 'Add subjects to track'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="badge badge-cyan">12 Available</span>
      </div>
      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3">
        {QUICK_TOOLS.map(({ to, icon: Icon, label, color, bg }) => (
          <Link
            ...
            <span className="text-xs font-semibold" style={{ color: 'var(--text-secondary)', fontFamily: 'Outfit' }}>
              {label}
            </span>
          </Link>
        ))}
